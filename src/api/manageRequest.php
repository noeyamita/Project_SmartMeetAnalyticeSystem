...
try {
    ...
    if ($action === 'approve') {
        $overlapStmt = $pdo->prepare("
            SELECT booking_id, attendees_count FROM Bookings
            WHERE room_id = ? AND booking_date = ? AND status = 1
            AND (start_time < ? AND end_time > ?)
        ");
        $overlapStmt->execute([$pending['room_id'], $pending['booking_date'], $pending['end_time'], $pending['start_time']]);
        $overlaps = $overlapStmt->fetchAll(PDO::FETCH_ASSOC);

        $displacedBookings = [];
        if (!empty($overlaps)) {
            $displacedIds = array_column($overlaps, 'booking_id');
            $placeholders = implode(',', array_fill(0, count($displacedIds), '?'));
