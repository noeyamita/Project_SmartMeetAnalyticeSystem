...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!alertBox) {
        console.warn('[ForgotPassword] ⚠️ ไม่พบ #alertBox ใน HTML — alert จะไม่แสดง!');
    }
...
function showAlert(message, type = 'error', duration = 4000) {
    ...
    const alertBox = document.getElementById('alertBox');
    if (!alertBox) {
        console.warn('[ForgotPassword] ⚠️ ไม่พบ #alertBox — กรุณาเพิ่ม <div id="alertBox"></div> ใน HTML');
        return;
    }
...
function setLoading(isLoading) {
    ...
    if (isLoading) {
        ...
        btn.textContent = btn.dataset.orig || 'Send Reset Link';
        btn.disabled = false;
    }
}
...
function showSuccess(email) {
    ...
    if (successEl) {
        ...
        successEl.innerHTML = `<p>✅ ส่งรหัสผ่านชั่วคราวไปที่ <strong>${masked}</strong> แล้ว<br>กรุณาตรวจสอบกล่องจดหมาย (รวมถึงโฟลเดอร์ Spam)</p>`;
    } else {
