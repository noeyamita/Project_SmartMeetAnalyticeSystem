...
function renderUsers(users) {
    ...
    list.innerHTML = users.map(u => {
        ...
        const banned = u.is_banned == 1 ? '<span class="banned-tag">ถูกแบน</span>' : '';
        return `
...
function selectUser(userId) {
    ...
    const initials = (selectedUser.fname?.[0] ?? '') + (selectedUser.lname?.[0] ?? '');
    document.getElementById('selectedAvatar').textContent = initials;
    document.getElementById('selectedName').textContent = `${selectedUser.fname} ${selectedUser.lname}`;
...
async function saveRole() {
    ...
    try {
        const res = await fetch(API_UPDATE, {
            ...
            body: JSON.stringify({ user_id: selectedUser.user_id, role_id: selectedRoleId })
        });
        const data = await res.json();
        ...
        if (data.success) {
            ...
            allUsers = allUsers.map(u => u.user_id == selectedUser.user_id ? { ...u, ...selectedUser } : u);
            renderUsers(filterUsers());
