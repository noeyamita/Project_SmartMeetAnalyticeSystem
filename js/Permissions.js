...
function displayRoles(roles) {
    ...
    if (!roles || roles.length === 0) {
        rolesList.innerHTML = '<div class="empty-state"><div class="empty-icon">📋</div><p>ไม่มีข้อมูลตำแหน่ง</p></div>';
        return;
    }
...
function displayMenuItems(permissions) {
    ...
    if (!permissions || permissions.length === 0) {
        menuGrid.innerHTML = '<p style="text-align:center; color: #999;">ไม่มีข้อมูลเมนู</p>';
        return;
    }
...
async function selectRole(roleId, roleName) {
    ...
    // Update title
    document.getElementById('permissionTitle').textContent = `จัดการสิทธิ์: ${roleName}`;
...
async function savePermissions() {
    ...
    try {
        ...
        if (result.success) {
            showToast(`บันทึกสิทธิ์สำเร็จ (${result.permissions_count} เมนู)`, 'success');
...
async function saveRole() {
    ...
    try {
        ...
        if (result.success) {
            showToast(editingRoleId ? 'แก้ไขตำแหน่งสำเร็จ' : 'เพิ่มตำแหน่งสำเร็จ', 'success');
            closeRoleModal();
            await loadRoles();
        } else {
...
function showDeleteRoleModal(roleId, roleName) {
    deleteRoleId = roleId;
    document.getElementById('deleteMessage').textContent = `คุณต้องการลบตำแหน่ง "${roleName}" หรือไม่?`;
...
async function confirmDeleteRole() {
    ...
    try {
        ...
        if (result.success) {
            ...
            // ถ้าลบ role ที่กำลังเลือกอยู่ ให้ reset
            if (currentRoleId === deleteRoleId) {
                currentRoleId = null;
                currentRoleName = null;
                document.getElementById('permissionTitle').textContent = 'เลือกตำแหน่งเพื่อจัดการสิทธิ์';
